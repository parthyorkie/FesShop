...
import { generatePresignedUrl } from '../services/chat.service';
...
export const getMessageHistory = asyncHandler(async (req: Request, res: Response) => {
  ...
  const messages = after
    ? await chatService.getMessageHistory(conversationId, userId, limit, after)
    : await chatService.getMessageHistory(conversationId, userId, limit, cursor);
...
export const createPresignedUrlHandler = asyncHandler(async (req: Request, res: Response) => {
  const userId = req.user?.userId;
  if (!userId) {
    throw createApiError(401, 'Unauthorized');
  }
  ...
  if (!conversationId || !mimeType || !size) {
    throw createApiError(400, 'conversationId, mimeType and size are required');
  }
  // Validate member
  await chatService.validateConversationMembership(conversationId, userId);
  const token = req.headers.authorization?.split(' ')[1];
  const keyPrefix = token?.split('.')[0];
  const { uploadUrl, key } = await generatePresignedUrl(conversationId, userId, mimeType, size, keyPrefix);
  res.status(200).json(createApiResponse(200, { uploadUrl, key }, 'Presigned URL generated'));
});
